...
class QEFFBaseModel(ABC):
    ...
    def _export_via_weightfree(
        ...
        example_inputs = {**ordered_inputs, **{key: value for key, value in example_inputs.items() if key not in sig_key_set}}
        dynamic_shapes = {**ordered_shapes, **{key: value for key, value in dynamic_shapes.items() if key not in sig_key_set}}
