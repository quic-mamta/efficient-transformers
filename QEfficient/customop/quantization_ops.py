...
def _isolate_cast_to_uint4_graph_inputs(model: ModelProto) -> bool:
    ...
    for node in model.graph.node:
        ...
        if isolated_name is None:
            ...
            rewritten_nodes.append(helper.make_node("Identity", [input_name], [isolated_name], name=f"{isolated_name}_identity"))
...
def _const_int32_node(name: str, values: list[int]):
    return helper.make_node("Constant", [], [name], value=helper.make_tensor(f"{name}_value", TensorProto.INT32, [len(values)], values))
...
def _rewrite_kimi_int4_extdata_to_int32_dequant(model: ModelProto) -> bool:
    ...
    for node_index, node in enumerate(model.graph.node):
        if node.domain == "com.qti.aisw.onnx" and node.op_type == "CastToUInt4" and node.input and node.input[0] in kimi_inputs:
    ...
    if not any(node.domain == "com.qti.aisw.onnx" and node.op_type == "CastToUInt4" for node in model.graph.node):
        kept_functions = [fn for fn in model.functions if not (fn.domain == "com.qti.aisw.onnx" and fn.name == "CastToUInt4")]
        del model.functions[:]
