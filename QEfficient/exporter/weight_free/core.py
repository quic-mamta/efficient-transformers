...
def _target_export_dtype(config) -> torch.dtype:
    """Return the floating-point dtype used by the exported model."""
...
def _build_meta_qeff_model(qeff_model):
    ...
    is_kimi_k25 = getattr(config, "model_type", None) == "kimi_k25"
    if is_kimi_k25:
        from QEfficient.transformers.models.modeling_auto import _build_kimi_k25_config_model

        meta_model = _build_kimi_k25_config_model(model_ref, {"config": config})
    else:
        with init_empty_weights():
            meta_model = qeff_model._hf_auto_class.from_config(config, attn_implementation="eager")

    if quant_config is None:
        meta_model = meta_model.to(dtype=_target_export_dtype(config))
    ...
    if quant_config is not None and not is_kimi_k25:
...
def _unresolved_meta_initializer_names(onnx_program) -> List[str]:
    """Return remaining meta/fake initializers that cannot be serialized."""
    from torch._subclasses.fake_tensor import FakeTensor

    names = []
    for name, initializer in onnx_program.model.graph.initializers.items():
        const_value = getattr(initializer, "const_value", None)
        raw_value = getattr(const_value, "raw", None)
        if isinstance(raw_value, FakeTensor) or (isinstance(raw_value, torch.Tensor) and raw_value.is_meta):
            names.append(name)
    return names
...
def export_weight_free_onnx(
    ...
    with export_context:
        ...
        target_dtype = _target_export_dtype(qeff_model.model.config)
        ...
        unresolved_initializers = _unresolved_meta_initializer_names(onnx_program)
        if unresolved_initializers:
            preview = ", ".join(unresolved_initializers[:20])
            raise RuntimeError(f"Unresolved weight-free meta initializers remain after promotion: {preview}")
