...
def find_checkpoint_key(
    ...
    for wrapper_prefix in ("base_model.", "model."):
        if onnx_name.startswith(wrapper_prefix):
            candidates.append(onnx_name[len(wrapper_prefix) :])
    if onnx_name.startswith("lm_head."):
        candidates.append(f"language_model.{onnx_name}")
