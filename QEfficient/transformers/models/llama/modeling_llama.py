...
def qeff_apply_rotary_pos_emb_single_w(x, cos, sin, position_ids=None, unsqueeze_dim=1):
...
def qeff_apply_rotary_pos_emb_single(x, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            #print(f"updating past key values with key_states= {key_states.shape}, value_states = {value_states.shape}") 
        ...
        query_states = qeff_apply_rotary_pos_emb_single_w(query_states, query_cos, query_sin)
        key_states = qeff_apply_rotary_pos_emb_single_w(key_states, key_cos, key_sin)
    ...
    def forward(
        ...
        if use_cache:  # kept for BC (cache positions)
            if not isinstance(past_key_values, StaticCache):
                num_window_length = past_key_values[0].shape[-2]-1
                num_heavy_hitter_tokens = num_window_length//2
                past_key_values = HHCache.from_legacy_cache(num_window_length, num_heavy_hitter_tokens, past_key_values)
