    ...
    def export(
        ...
        **kwargs :
        ...
        if use_weight_free_export and layerwise:
            raise NotImplementedError("Weight-free image-text export uses dual-QPC dynamo export, not layerwise export.")
...
class _QEFFAutoModelForImageTextToTextSingleQPC(QEFFTransformersBase, MultimodalUtilityMixin):
    ...
    def export(
        ...
        **kwargs :
        ...
        """
        if use_weight_free_export:
            raise NotImplementedError("Weight-free image-text export is supported only for dual QPC. Use kv_offload=True.")
        if prefill_only:
            assert prefill_seq_len > 1
            if not enable_chunking and self.continuous_batching:
                raise NotImplementedError(
...
class QEFFAutoModelForImageTextToText:
    ...
    def __new__(
        ...
        **kwargs :
        ...
        """
        if kwargs.get("allow_config_only_quantized") and not kv_offload:
            raise NotImplementedError("Weight-free image-text export is supported only for dual QPC. Use kv_offload=True.")
        if kv_offload:
        ...
        **kwargs :
        ...
        if use_weight_free_export:
            if kv_offload is False:
                raise NotImplementedError(
                    "QEFFAutoModelForImageTextToText weight-free export is supported only for dual QPC. "
                    "Use kv_offload=True or omit kv_offload."
                )
            kv_offload = True
        ...
        instance = cls(
            ...
            pretrained_model_name_or_path=pretrained_model_name_or_path,
            qaic_config=qaic_config,
            allow_config_only_quantized=use_weight_free_export,
            **kwargs,
        )
        if use_weight_free_export:
            instance._weight_free_config_only = True
