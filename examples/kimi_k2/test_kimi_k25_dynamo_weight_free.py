...
def _make_args(device_ids: list[int] | None = None) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        prompt=os.environ.get("KIMI_K25_DYNAMO_WEIGHT_FREE_PROMPT", os.environ.get("KIMI_K25_DYNAMO_PROMPT", TEXT_PROMPT)),
        ...
        image_height=_env_int_any(
            None, "KIMI_K25_DYNAMO_WEIGHT_FREE_IMAGE_HEIGHT", "KIMI_K25_DYNAMO_IMAGE_HEIGHT"
        ),
        ...
        mxfp6_matmul=_env_bool_any(
            False, "KIMI_K25_DYNAMO_WEIGHT_FREE_MXFP6_MATMUL", "KIMI_K25_DYNAMO_MXFP6_MATMUL"
        ),
...
def _materialize_weight_free_extdata(component_model, component_dir: Path, onnx_name: str):
    ...
    if missing:
        raise RuntimeError(f"Weight-free extdata tensors are missing from {component_model.__class__.__name__}: {missing}")
...
def check_kimi_k25_dynamo_weight_free_hf_vs_qaic():
    ...
    print(f"Weight-free Dynamo QAIC compile time: {compile_end - compile_start:.2f} sec (skip_compile={args.skip_compile})")
